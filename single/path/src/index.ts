...
function isSimplePath(val: string) {
  return !val.includes('*')
    && !val.includes('~')
    && !val.includes('[')
    && !val.includes(']')
...
function getIn(segments: Segments, source: any) {
...
function setIn(segments: Segments, source: any, value: any) {
  for (let i = 0; i < segments.length; i++) {
    const index = segments[i]
    const rules = getDestructor(index as string)
    if (!rules) {
...
function deleteIn(segments: Segments, source: any) {
  for (let i = 0; i < segments.length; i++) {
    const index = segments[i]
    const rules = getDestructor(index as string)
    if (!rules) {
...
function existIn(segments: Segments, source: any, start: number | Path) {
...
function parse(pattern: Pattern, base?: Pattern) {
...
function parseString(source: any) {
  if (isStr(source)) {
...
export class Path {
  ...
  constructor(input: Pattern, base?: Pattern) {
    const { tree, segments, entire, isRegExp, isMatchPattern, isWildMatchPattern, haveRelativePattern, haveExcludePattern } = parse(input, base)
    this.entire = entire
